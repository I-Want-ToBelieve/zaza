          ...
          .map(([k, v]) => {
            ...
            return {
              ...
              params: v.parameters
                ...
                .map(([type, name]) => {
                  const doc: string = v.parameters_doc[name] ?? ''
                  const matched = (doc.match(/^\((.+?)\)/)?.[1] ?? '').replace(
                    '{number, number}',
                    '[number+number]'
                  )

                  const array = matched.split(',')
                  const default_ = array.find((it) => it.includes('default='))
                  ...
                  return {
                    ...
                    type:
                      type === ''
                        ? array.filter(
                          (it) =>
                            !['optional', 'default=', 'not yet'].some((it_) =>
                              it.includes(it_)
                            )
                        )[0] ?? 'any'
                        : type.includes('(')
                    ...
                    optional:
                      ...
                      array.some((it) => it.includes('nil')) ||
                      type.includes('*'),
                    isRest: name === '...',
                    default: default_ ? default_.split('=')[1] : void 0,
                  }
                }),
              doc: v.doc.join('\n'),
            ...
            ...funs_.map((fun, idx) => {
                  ...
                  fun.params.map((param) => {
                    ...
                    return ts.factory.createParameterDeclaration(
                      ...
                      param.isRest
                        ...
                        : ts.factory.createTypeReferenceNode(
                          ts.factory.createIdentifier(
                            (param.type === '' ? 'any' : param.type)
                              .replace('[number+number]', '[number, number]')
                              .replace(' or ', '|')
                              .replace('|fun(name: string', 'Function')
                              .replace('Object', 'any')
                              .toLowerCase()
                          ),
                          void 0
              ...
              ts.addSyntheticLeadingComment(
                ...
                '*\n*' +
                  ...
                  fun.params
                    .map((param) => {
                      return '*@param ' + param.name + ' - ' + param.doc + '\n'
                    })
                    .join('') +
                  '\n',
                /* hasTrailingNewLine */ true
